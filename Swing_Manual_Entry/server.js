...
const CFG = {
  BRAIN_NAME: envStr("BRAIN_NAME", "BrainFVVO_Swing_XRP_v1m_FEE_AWARE_PROFIT_LOCK_LIVE"),
  ...
  STATE_FILE_NAME: envStr("STATE_FILE_NAME", "brainfvvo-swing-xrp-v1k-paper-live-state.json"),
  ...
  BREAKOUT_RETEST_RECLAIM_ZONE_MAX_ENTRY_ABOVE_HIGH_PCT: envNum("BREAKOUT_RETEST_RECLAIM_ZONE_MAX_ENTRY_ABOVE_HIGH_PCT", 0.154),
  ...
  // v1k: retain v1j no-chase wait and use the replay-selected tighter XRP cap (0.154%).
  // keep the setup alive without chasing. It may enter later only if price returns
  // inside the original max-entry window and all existing recovery gates pass.
  // Set to "cancel" to restore the v1i behaviour.
  ...
  // v1m: interpret the minimum lock as desired estimated NET profit. The
  // signal-reference floor adds configured round-trip cost before execution.
...
function defaultState() {
  return {
    ...
    priceEntry: { pending: null, pending2: null, pending3: null, last: null },
...
function ensurePriceEntryState() {
  if (!state.priceEntry || typeof state.priceEntry !== "object") state.priceEntry = { pending: null, pending2: null, pending3: null, last: null };
