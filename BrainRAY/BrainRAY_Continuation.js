...
/**
 * BrainRAY_Continuation_v3.2
 *
 * v3.2:
 * - dedicated post-exit continuation re-entry path
 * - elevated continuation path after fresh bull trend change
 * - ray conflict guard for same-bar opposite events
 * - keeps v3.0/v3.1 FVVO and fast tick launch structure
 */
...
const CONFIG = {
  ...
  BRAIN_NAME: s(process.env.BRAIN_NAME, "BrainRAY_Continuation_v3.2"),
  ...
  POST_EXIT_CONTINUATION_MIN_RSI: n(process.env.POST_EXIT_CONTINUATION_MIN_RSI, 62),
  POST_EXIT_CONTINUATION_MIN_ADX: n(process.env.POST_EXIT_CONTINUATION_MIN_ADX, 26),
  POST_EXIT_CONTINUATION_STRONG_MIN_RSI: n(process.env.POST_EXIT_CONTINUATION_STRONG_MIN_RSI, 68),
  POST_EXIT_CONTINUATION_STRONG_MIN_ADX: n(process.env.POST_EXIT_CONTINUATION_STRONG_MIN_ADX, 45),
  POST_EXIT_CONTINUATION_MAX_CHASE_PCT: n(process.env.POST_EXIT_CONTINUATION_MAX_CHASE_PCT, 0.42),
  POST_EXIT_CONTINUATION_STRONG_MAX_CHASE_PCT: n(process.env.POST_EXIT_CONTINUATION_STRONG_MAX_CHASE_PCT, 0.75),
  POST_EXIT_CONTINUATION_MAX_EXT_FROM_EMA18_PCT: n(process.env.POST_EXIT_CONTINUATION_MAX_EXT_FROM_EMA18_PCT, 1.1),
  ...
  RAY_CONFLICT_GUARD_SEC: n(process.env.RAY_CONFLICT_GUARD_SEC, 30),
...
function buildInitialRuntimeState() {
  return {
    ...
    ray: {
      ...
      conflictLock: false,
      conflictLockUntilBar: null,
      conflictReason: null,
      conflictFeatureConfirmNeededUntilBar: null,
      lastBullEventAt: null,
      lastBearEventAt: null,
      lastBullEventName: null,
      lastBearEventName: null,
    },
    ...
    reentry: {
      ...
      bullRegimeId: null,
      exitReason: null,
      postExitContinuationEligible: false,
      postExitContinuationUntilBar: null,
      postExitPeakPnlPct: null,
    },
...
const S = { ...buildInitialRuntimeState(), logs: [] };
...
function ema8SlopePct() {
  ...
  const cur = S.lastFeature;
  if (!prev || !cur) return NaN;
  if (!Number.isFinite(prev.ema8) || !Number.isFinite(cur.ema8) || prev.ema8 === 0) return NaN;
  return pctDiff(prev.ema8, cur.ema8);
}
...
function clearReentry(reason = "reset") {
  if (S.reentry.eligible || S.reentry.postExitContinuationEligible) log("🔁 REENTRY_DISABLED", { reason });
  S.reentry = {
    eligible: false,
    eligibleUntilBar: null,
    eligibleFromBar: null,
...
function clearRayConflict(reason = "cleared") {
  if (S.ray.conflictLock) log("⚖️ RAY_CONFLICT_CLEARED", { reason });
  S.ray.conflictLock = false;
  S.ray.conflictLockUntilBar = null;
  S.ray.conflictReason = null;
  S.ray.conflictFeatureConfirmNeededUntilBar = null;
}
function armRayConflict(reason) {
  S.ray.conflictLock = true;
  S.ray.conflictLockUntilBar = S.barIndex + CONFIG.RAY_CONFLICT_CONFIRM_FEATURE_BARS;
  S.ray.conflictReason = reason;
  S.ray.conflictFeatureConfirmNeededUntilBar = S.barIndex + CONFIG.RAY_CONFLICT_CONFIRM_FEATURE_BARS;
  clearTrendChangeLaunch("ray_conflict_lock");
  clearFastTickLaunch("ray_conflict_lock");
  clearBreakoutMemory("ray_conflict_lock");
  log("⚖️ RAY_CONFLICT_ARMED", {
    reason,
    barIndex: S.barIndex,
    untilBar: S.ray.conflictLockUntilBar,
  });
}
...
// ray regime
// --------------------------------------------------
function maybeArmRayConflict(ts, incomingType) {
  if (!CONFIG.RAY_CONFLICT_GUARD_ENABLED) return false;
  const recentBull = ageSec(S.ray.lastBullEventAt) <= CONFIG.RAY_CONFLICT_GUARD_SEC;
  const recentBear = ageSec(S.ray.lastBearEventAt) <= CONFIG.RAY_CONFLICT_GUARD_SEC;
  if (incomingType === "bear" && recentBull) {
    armRayConflict(`bear_after_${S.ray.lastBullEventName || 'bull'}_within_${CONFIG.RAY_CONFLICT_GUARD_SEC}s`);
    return true;
  }
  if (incomingType === "bull" && recentBear) {
    armRayConflict(`bull_after_${S.ray.lastBearEventName || 'bear'}_within_${CONFIG.RAY_CONFLICT_GUARD_SEC}s`);
    return true;
  }
  return false;
}
function turnBullRegimeOn(ts, source) {
  if (S.ray.conflictLock) {
    log("🟡 BULL_REGIME_ON_DELAYED_BY_CONFLICT", {
      source,
      ts,
      untilBar: S.ray.conflictLockUntilBar,
      conflictReason: S.ray.conflictReason,
    });
    return;
  }
...
function handleRayEvent(body) {
  ...
  if (/Bullish Trend Change/i.test(name) && CONFIG.RAY_USE_BULLISH_TREND_CHANGE) {
    ...
    S.ray.lastBullEventAt = ts;
    S.ray.lastBullEventName = "bullish_trend_change";
    const conflict = maybeArmRayConflict(ts, "bull");
    turnBullRegimeOn(ts, "ray_bullish_trend_change");
    log("🟢 RAY_BULLISH_TREND_CHANGE", { price, ts, conflictLocked: conflict });
    if (CONFIG.TREND_CHANGE_LAUNCH_ENABLED && !S.ray.conflictLock) {
  ...
  if (/Bullish Trend Continuation/i.test(name) && CONFIG.RAY_USE_BULLISH_TREND_CONTINUATION) {
    ...
    S.ray.lastBullEventAt = ts;
    S.ray.lastBullEventName = "bullish_trend_continuation";
    const conflict = maybeArmRayConflict(ts, "bull");
    if (!S.ray.bullContext) turnBullRegimeOn(ts, "ray_bullish_trend_continuation");
    log("🟩 RAY_BULLISH_TREND_CONTINUATION", { price, ts, conflictLocked: conflict });
    const decision = tryEntry("ray_bullish_trend_continuation", body);
    if (!decision.allow && CONFIG.FAST_TICK_LAUNCH_ENABLED && !S.ray.conflictLock) {
  ...
  if (/Bullish BOS/i.test(name) && CONFIG.RAY_USE_BULLISH_BOS) {
    ...
    S.ray.lastBullEventAt = ts;
    S.ray.lastBullEventName = "bullish_bos";
  ...
  if (/Bearish Trend Change/i.test(name) && CONFIG.RAY_USE_BEARISH_TREND_CHANGE) {
    ...
    S.ray.lastBearEventAt = ts;
    S.ray.lastBearEventName = "bearish_trend_change";
    const conflict = maybeArmRayConflict(ts, "bear");
    log("🔴 RAY_BEARISH_TREND_CHANGE", { price, ts, conflictLocked: conflict });
  ...
  if (/Bearish Trend Continuation/i.test(name) && CONFIG.RAY_USE_BEARISH_TREND_CONTINUATION) {
    ...
    S.ray.lastBearEventAt = ts;
    S.ray.lastBearEventName = "bearish_trend_continuation";
    const conflict = maybeArmRayConflict(ts, "bear");
    log("🟥 RAY_BEARISH_TREND_CONTINUATION", { price, ts, conflictLocked: conflict });
...
function updateBarProgress(ts) {
  ...
  if (key !== S.lastBarKey) {
    ...
    invalidateRayConflict();
  }
}
function invalidateRayConflict() {
  if (!S.ray.conflictLock) return;
  if (S.barIndex > n(S.ray.conflictLockUntilBar, -1)) clearRayConflict("expired_without_confirm");
}
function handleFeature(body) {
  ...
  confirmRayConflictFromFeature(feature);
...
function confirmRayConflictFromFeature(feature) {
  if (!S.ray.conflictLock) return;
  const emaBullOk = Number.isFinite(feature.ema8) && Number.isFinite(feature.ema18) && feature.ema8 >= feature.ema18;
  const closeAboveEma8 = Number.isFinite(feature.close) && Number.isFinite(feature.ema8) && feature.close >= feature.ema8;
  const rsiOk = !Number.isFinite(feature.rsi) || feature.rsi >= CONFIG.MIN_RSI_LONG;
  if (emaBullOk && closeAboveEma8 && rsiOk) {
    clearRayConflict("feature_confirmed_recovery");
    turnBullRegimeOn(feature.time, "feature_confirmed_after_conflict");
  }
}
function evaluateStructureAndArmMemory(f) {
  ...
  const bullRayContext =
    S.ray.bullContext || ageSec(S.ray.lastBullTrendChangeAt) < 3600 || ageSec(S.ray.lastBullTrendContinuationAt) < 1800;
  ...
  if (!structureOk || S.ray.conflictLock) return;
  S.breakoutMemory = {
    active: true,
    used: false,
    armedBar: S.barIndex,
...
function invalidateReentry() {
  if (S.reentry.eligible && S.barIndex > n(S.reentry.eligibleUntilBar, -1)) clearReentry("expired");
  if (S.reentry.postExitContinuationEligible && S.barIndex > n(S.reentry.postExitContinuationUntilBar, -1)) {
    clearReentry("post_exit_continuation_expired");
  }
}
...
function tryEntry(source, body) {
  ...
  if (!decision.allow) {
    if (["deferred_trend_change_launch", "immediate_trend_change_launch", "tick_confirmed_fast_launch"].includes(source)) {
...
function evaluateEntry(source, body) {
  ...
  if (!["feature_reentry", "post_exit_continuation_reentry"].includes(source)) {
    reasonPush(reasons, !S.ray.bullContext, "no_bull_context");
  }
  if (S.ray.conflictLock && !["feature_reentry", "post_exit_continuation_reentry"].includes(source)) {
    reasonPush(reasons, true, "ray_conflict_lock_active");
  }
  ...
  if (source === "tick_confirmed_fast_launch" && CONFIG.FAST_TICK_LAUNCH_ENABLED) {
    ...
    const strongFastLaunch = Number.isFinite(rsi) && Number.isFinite(adx) && rsi >= CONFIG.FAST_TICK_LAUNCH_STRONG_MIN_RSI && adx >= CONFIG.FAST_TICK_LAUNCH_STRONG_MIN_ADX;
    let allowedChase = strongFastLaunch ? CONFIG.FAST_TICK_LAUNCH_STRONG_MAX_CHASE_PCT : CONFIG.FAST_TICK_LAUNCH_MAX_CHASE_PCT;
    ...
    if (rr.length === 0) {
      return { allow: true, source, mode: strongFastLaunch ? "tick_confirmed_launch_long_strong" : "tick_confirmed_launch_long", entryPrice: px, extFromEma8, extFromEma18, ticksAboveConfirm: tl.ticksAboveConfirm, confirmPrice: round4(tl.confirmPrice), strongFastLaunch, fvvo: fv };
    }
    return { allow: false, source, reasons: rr, extFromEma8, extFromEma18, ticksAboveConfirm: tl.ticksAboveConfirm, confirmPrice: round4(tl.confirmPrice), strongFastLaunchCandidate: strongFastLaunch, fvvo: fv };
  }

  // trend change launch / elevated continuation
  if (["immediate_trend_change_launch", "deferred_trend_change_launch", "ray_bullish_trend_continuation"].includes(source)) {
    ...
    const isContinuation = source === "ray_bullish_trend_continuation";
    const minLaunchRsiBase = isDeferred ? CONFIG.DEFERRED_LAUNCH_MIN_RSI : CONFIG.TREND_CHANGE_LAUNCH_MIN_RSI;
    const minLaunchRsi = Math.max(0, minLaunchRsiBase - (bullishFvvo ? CONFIG.FVVO_LAUNCH_RSI_RELAX : 0));
    const minLaunchAdx = isDeferred ? CONFIG.DEFERRED_LAUNCH_MIN_ADX : CONFIG.TREND_CHANGE_LAUNCH_MIN_ADX;

    const strongOverride = CONFIG.STRONG_LAUNCH_OVERRIDE_ENABLED && Number.isFinite(rsi) && Number.isFinite(adx) && rsi >= CONFIG.STRONG_LAUNCH_MIN_RSI && adx >= CONFIG.STRONG_LAUNCH_MIN_ADX && extFromEma18 <= CONFIG.STRONG_LAUNCH_MAX_EXT_FROM_EMA18_PCT && launchChasePct <= CONFIG.STRONG_LAUNCH_MAX_CHASE_PCT;
    const slowRampOverride = isDeferred && CONFIG.DEFERRED_SLOW_RAMP_OVERRIDE_ENABLED && Number.isFinite(rsi) && Number.isFinite(adx) && emaBullOk && closeAboveEma8Ok && rsi >= CONFIG.DEFERRED_SLOW_RAMP_MIN_RSI && adx >= CONFIG.DEFERRED_SLOW_RAMP_MIN_ADX && launchChasePct <= CONFIG.DEFERRED_SLOW_RAMP_MAX_CHASE_PCT && extFromEma18 <= CONFIG.DEFERRED_SLOW_RAMP_MAX_EXT_FROM_EMA18_PCT;

    const freshBullWindowBars = S.ray.bullRegimeStartedAt ? S.barIndex - (S.trendChangeLaunch.armedBar ?? S.barIndex) : 999;
    const emaSlope = ema8SlopePct();
    const elevatedStrongEnough = Number.isFinite(rsi) && Number.isFinite(adx) && rsi >= CONFIG.ELEVATED_CONTINUATION_MIN_RSI && adx >= CONFIG.ELEVATED_CONTINUATION_MIN_ADX;
    const elevatedFvvoOk = CONFIG.ELEVATED_CONTINUATION_ALLOW_NEGATIVE_FVVO_IF_STRONG ? (!negativeFvvo || elevatedStrongEnough) : !negativeFvvo;
    const elevatedContinuationOk = CONFIG.ELEVATED_CONTINUATION_ENABLED && isContinuation && S.ray.bullContext && emaBullOk && (!CONFIG.ELEVATED_CONTINUATION_REQUIRE_CLOSE_ABOVE_EMA8 || closeAboveEma8Ok) && Number.isFinite(emaSlope) && emaSlope >= CONFIG.ELEVATED_CONTINUATION_MIN_EMA8_SLOPE_PCT && Number.isFinite(rsi) && rsi >= CONFIG.ELEVATED_CONTINUATION_MIN_RSI && Number.isFinite(adx) && adx >= CONFIG.ELEVATED_CONTINUATION_MIN_ADX && launchChasePct <= CONFIG.ELEVATED_CONTINUATION_MAX_CHASE_PCT && extFromEma18 <= CONFIG.ELEVATED_CONTINUATION_MAX_EXT_FROM_EMA18_PCT && elevatedFvvoOk && freshBullWindowBars <= CONFIG.ELEVATED_CONTINUATION_WINDOW_BARS;

    const rr = [];
    let allowedLaunchChase = strongOverride ? CONFIG.STRONG_LAUNCH_MAX_CHASE_PCT : CONFIG.TREND_CHANGE_LAUNCH_MAX_CHASE_PCT;
    ...
    const allowedLaunchExtEma18 = strongOverride ? CONFIG.STRONG_LAUNCH_MAX_EXT_FROM_EMA18_PCT : CONFIG.TREND_CHANGE_LAUNCH_MAX_EXT_FROM_EMA18_PCT;

    if (!elevatedContinuationOk) {
      if (!isContinuation) {
    ...
    if (elevatedContinuationOk) {
      return {
        allow: true,
        source,
        mode: "bullish_elevated_continuation_long",
        entryPrice: px,
        extFromEma8,
        extFromEma18,
        launchChasePct,
    ...
    if (rr.length === 0) {
      return {
        ...
        mode: strongOverride
          ...
          : isContinuation
          ? "bullish_trend_continuation_launch_long"
    ...
    return {
      ...
      elevatedContinuationCandidate: elevatedContinuationOk,
      isDeferredLaunch: isDeferred,
      minLaunchRsi,
      minLaunchAdx,
      fvvo: fv,
  ...
  // dedicated post-exit continuation path
  if (source === "post_exit_continuation_reentry") {
    const rr = [];
    reasonPush(rr, !CONFIG.POST_EXIT_CONTINUATION_ENABLED, "post_exit_continuation_disabled");
    reasonPush(rr, !S.reentry.postExitContinuationEligible, "post_exit_continuation_not_eligible");
    reasonPush(rr, S.barIndex > n(S.reentry.postExitContinuationUntilBar, -1), "post_exit_continuation_expired");
    reasonPush(rr, S.reentry.bullRegimeId !== S.ray.bullRegimeId, "post_exit_continuation_regime_mismatch");
    reasonPush(rr, !S.ray.bullContext, "post_exit_continuation_no_bull_context");
    reasonPush(rr, !emaBullOk, "post_exit_continuation_ema_invalid");
    reasonPush(rr, !closeAboveEma8Ok, "post_exit_continuation_close_below_ema8");
    ...
    const strongPostExit = Number.isFinite(rsi) && Number.isFinite(adx) && rsi >= CONFIG.POST_EXIT_CONTINUATION_STRONG_MIN_RSI && adx >= CONFIG.POST_EXIT_CONTINUATION_STRONG_MIN_ADX;
    let allowedChase = strongPostExit ? CONFIG.POST_EXIT_CONTINUATION_STRONG_MAX_CHASE_PCT : CONFIG.POST_EXIT_CONTINUATION_MAX_CHASE_PCT;
    if (bullishFvvo) allowedChase += CONFIG.FVVO_REENTRY_MAX_CHASE_BONUS_PCT;
    if (!(CONFIG.POST_EXIT_CONTINUATION_IGNORE_SNIPER_SELL_IF_STRONG && strongPostExit) && fv.snap.sniperSell) {
      allowedChase -= CONFIG.FVVO_SNIPER_SELL_CHASE_PENALTY_PCT;
    }
    if (fv.snap.burstBearish) allowedChase -= CONFIG.FVVO_BURST_BEARISH_CHASE_PENALTY_PCT;

    const minRsi = Math.max(0, CONFIG.POST_EXIT_CONTINUATION_MIN_RSI - (bullishFvvo ? CONFIG.FVVO_REENTRY_RSI_RELAX : 0));
    reasonPush(rr, Number.isFinite(rsi) && rsi < minRsi, "post_exit_continuation_rsi_too_low");
    reasonPush(rr, Number.isFinite(adx) && adx < CONFIG.POST_EXIT_CONTINUATION_MIN_ADX, "post_exit_continuation_adx_too_low");
    reasonPush(rr, chasePct > allowedChase, "post_exit_continuation_chase_too_high");
    reasonPush(rr, extFromEma18 > CONFIG.POST_EXIT_CONTINUATION_MAX_EXT_FROM_EMA18_PCT, "post_exit_continuation_too_extended_from_ema18");
    if (CONFIG.POST_EXIT_CONTINUATION_BLOCK_ON_BURST_BEARISH) {
      reasonPush(rr, fv.snap.burstBearish && !strongPostExit, "post_exit_continuation_burst_bearish_block");
    }
    ...
    if (rr.length === 0) {
      return {
        allow: true,
        source,
        mode: strongPostExit ? "post_exit_continuation_reentry_long_strong" : "post_exit_continuation_reentry_long",
        entryPrice: px,
        chasePct,
    ...
    return {
      ...
      chasePct,
      anchor: round4(anchor),
      strongPostExitCandidate: strongPostExit,
      fvvo: fv,
    };
  }
  ...
  if (source === "feature_reentry" || (CONFIG.PHASE2_REENTRY_ENABLED && S.reentry.eligible)) {
    ...
    reasonPush(rr, (useFast ? CONFIG.FAST_REENTRY_REQUIRE_BULL_CONTEXT : CONFIG.REENTRY_REQUIRE_BULL_CONTEXT) && !S.ray.bullContext, "reentry_no_bull_context");
    ...
    const reentryRequireCloseAboveEma8 = useFast ? CONFIG.FAST_REENTRY_REQUIRE_CLOSE_ABOVE_EMA8 : CONFIG.REENTRY_REQUIRE_CLOSE_ABOVE_EMA8;
    const reentryCloseAboveEma8Ok = !reentryRequireCloseAboveEma8 || (Number.isFinite(close) && Number.isFinite(ema8) && close >= ema8);
    const anchor = Number.isFinite(S.reentry.anchorPrice) ? S.reentry.anchorPrice : ema8;
    ...
    const minRsi = Math.max(0, minRsiBase - (bullishFvvo ? CONFIG.FVVO_REENTRY_RSI_RELAX : 0));
    const minAdx = useFast ? CONFIG.FAST_REENTRY_MIN_ADX : CONFIG.MIN_ADX_CONTINUATION;
    const strongReentryOverride = CONFIG.STRONG_REENTRY_OVERRIDE_ENABLED && S.ray.bullContext && emaBullOk && reentryCloseAboveEma8Ok && Number.isFinite(rsi) && Number.isFinite(adx) && rsi >= CONFIG.STRONG_REENTRY_MIN_RSI && adx >= CONFIG.STRONG_REENTRY_MIN_ADX && reentryChasePct <= CONFIG.STRONG_REENTRY_MAX_CHASE_PCT;
    let allowedReentryChase = strongReentryOverride ? CONFIG.STRONG_REENTRY_MAX_CHASE_PCT : baseMaxReentryChase;
    ...
    if (rr.length === 0) {
      return { allow: true, source, mode: strongReentryOverride ? "feature_pullback_reclaim_reentry_long_strong" : "feature_pullback_reclaim_reentry_long", entryPrice: px, reentryChasePct, anchor: round4(anchor), bullRegimeId: S.ray.bullRegimeId, strongReentryOverride, fvvo: fv };
    }
    return { allow: false, source, reasons: rr, reentryChasePct, anchor: round4(anchor), bullRegimeId: S.ray.bullRegimeId, strongReentryOverrideCandidate: strongReentryOverride, fvvo: fv };
  }

  // standard continuation
  const contReasons = [];
  const contMinRsi = Math.max(0, CONFIG.MIN_RSI_LONG - (bullishFvvo ? CONFIG.FVVO_CONT_RSI_RELAX : 0));
  ...
  if (contReasons.length === 0) {
    return { allow: true, source, mode: bullishFvvo ? "breakout_continuation_long_fvvo" : "breakout_continuation_long", entryPrice: px, extFromEma8, extFromEma18, contChasePct, fvvo: fv };
  }
  ...
  if (memReasons.length === 0) {
    return { allow: true, source, mode: bullishFvvo ? "delayed_breakout_memory_long_fvvo" : "delayed_breakout_memory_long", entryPrice: px, memChasePct, memAnchor: round4(memAnchor), reclaimPrice: round4(mem.reclaimPrice), fvvo: fv };
  }

  return { allow: false, source, reasons: [...contReasons, ...memReasons], extFromEma8, extFromEma18, contChasePct, memChasePct, fvvo: fv };
}
function doEnter(mode, price, decision = {}) {
  ...
  if (["feature_pullback_reclaim_reentry_long", "feature_pullback_reclaim_reentry_long_strong", "post_exit_continuation_reentry_long", "post_exit_continuation_reentry_long_strong"].includes(mode)) {
    S.ray.reentryCountInRegime += 1;
    clearReentry("consumed_on_reentry");
  }

  if (["bullish_trend_change_launch_long", "bullish_trend_change_launch_long_strong", "bullish_trend_change_launch_long_slow_ramp", "bullish_elevated_continuation_long", "tick_confirmed_launch_long", "tick_confirmed_launch_long_strong"].includes(mode)) {
  ...
  forward3Commas("enter_long", price, { mode, setup_type: mode, brain: CONFIG.BRAIN_NAME }).catch((err) => {
...
function evaluateBarExit(feature) {
  ...
  const pnlPct = pctDiff(S.entryPrice, price);
  if (CONFIG.LOCAL_TP_EXIT_ENABLED && CONFIG.LOCAL_TP_EXIT_ON_CLOSE_BELOW_EMA8 && Number.isFinite(feature.ema8) && price < feature.ema8 && pnlPct >= CONFIG.LOCAL_TP_MIN_PROFIT_PCT) {
...
function markReentryEligible(reason, exitPrice) {
  ...
  const peakBeforeExit = S.peakPrice;
  const peakPnlPct = S.peakPnlPct;
  S.reentry = {
    eligible: true,
    ...
    bullRegimeId: S.ray.bullRegimeId,
    exitReason: reason,
    postExitContinuationEligible: CONFIG.POST_EXIT_CONTINUATION_ENABLED,
    postExitContinuationUntilBar: S.barIndex + CONFIG.POST_EXIT_CONTINUATION_WINDOW_BARS,
    postExitPeakPnlPct: peakPnlPct,
  };
  ...
  log("🔁 TP_EXIT_WAIT_REENTRY", {
    ...
    postExitContinuationUntilBar: S.reentry.postExitContinuationUntilBar,
    postExitPeakPnlPct: round4(peakPnlPct),
  });
}
function doExit(reason, price, ts, exitClass = "stop_exit") {
  ...
  forward3Commas("exit_long", exitPrice, { reason, brain: CONFIG.BRAIN_NAME, entry_mode: S.entryMode }).catch((err) => {
...
async function forward3Commas(action, triggerPrice, meta = {}) {
  ...
  if (!CONFIG.C3_SIGNAL_URL || !CONFIG.C3_SIGNAL_SECRET || !botUuid) {
    log("⚠️ 3COMMAS_CONFIG_MISSING", { hasUrl: !!CONFIG.C3_SIGNAL_URL, hasSecret: !!CONFIG.C3_SIGNAL_SECRET, hasBotUuid: !!botUuid });
    return;
  }
  ...
  try {
    ...
    log("✅ 3COMMAS_RESP", { action, status: res.status, ok: res.ok, body: text.slice(0, 500) });
  } finally {
    clearTimeout(t);
  }
}
...
// --------------------------------------------------
app.get("/", (_req, res) => {
  res.json({ ok: true, brain: CONFIG.BRAIN_NAME, symbol: CONFIG.SYMBOL, tf: CONFIG.ENTRY_TF, startedAt: S.startedAt });
});
...
app.post("/reset", (req, res) => {
  ...
  return res.json({ ok: true, reset: true, reason, brain: CONFIG.BRAIN_NAME, symbol: CONFIG.SYMBOL, tf: CONFIG.ENTRY_TF });
});
app.post(CONFIG.WEBHOOK_PATH, (req, res) => {
  ...
  try {
    if (parsed.family === "tick") return res.json(handleTick(body));
    if (parsed.family === "feature") {
      handleFeature(body);
      return res.json({ ok: true, kind: "feature", barIndex: S.barIndex, inPosition: S.inPosition });
    }
    if (parsed.family === "ray") {
      handleRayEvent(body);
      return res.json({ ok: true, kind: "ray", event: parsed.name, bullContext: S.ray.bullContext, inPosition: S.inPosition });
    }
    if (parsed.family === "fvvo") {
      handleFvvoEvent(body);
      return res.json({ ok: true, kind: "fvvo", event: parsed.name, fvvoScore: getFvvoScore() });
    }
