...
const CFG = {
  ...
  // v1c: execution transport can use the legacy Signal Bot JSON contract or
  // the new per-bot 3Commas V2 custom-code contract.
  C3_EXECUTION_SCHEMA: envStr("C3_EXECUTION_SCHEMA", "legacy_signal_bot").toLowerCase(),
...
function configProblems() {
  ...
  if (!["legacy_signal_bot", "v2_custom_code"].includes(CFG.C3_EXECUTION_SCHEMA)) problems.push("C3_EXECUTION_SCHEMA_INVALID");
  if (CFG.C3_EXECUTION_SCHEMA === "legacy_signal_bot") {
    if (!CFG.C3_SIGNAL_SECRET) problems.push("C3_SIGNAL_SECRET_REQUIRED");
    if (!getBotUuid()) problems.push("C3_BOT_UUID_REQUIRED");
  }
  if (CFG.C3_EXECUTION_SCHEMA === "v2_custom_code") {
  ...
  if (CFG.C3_EXECUTION_SCHEMA === "legacy_signal_bot" && CFG.C3_ENTRY_SIZE_SOURCE !== "bot_fixed") problems.push("C3_ENTRY_SIZE_SOURCE_MUST_BE_BOT_FIXED");
  if (CFG.C3_EXECUTION_SCHEMA === "legacy_signal_bot" && !CFG.C3_EXIT_INCLUDE_POSITION_ORDER) problems.push("C3_EXIT_INCLUDE_POSITION_ORDER_MUST_BE_TRUE");
...
async function forward3Commas(action, price, reason, options = {}) {
  ...
  log("INFO", "C3_FORWARD_SEND", { action, reason, symbol: CFG.SYMBOL, price, requestId, executionSchema: CFG.C3_EXECUTION_SCHEMA, c3Timestamp: body.timestamp || null, triggerPrice: body.trigger_price || null, hasOrder: Boolean(body.order), amountPerTrade: action === "enter_long" && CFG.C3_EXECUTION_SCHEMA === "v2_custom_code" ? CFG.C3_AMOUNT_PER_TRADE : null, amountPerTradeType: action === "enter_long" && CFG.C3_EXECUTION_SCHEMA === "v2_custom_code" ? CFG.C3_AMOUNT_PER_TRADE_TYPE : null, orderType: action === "enter_long" && CFG.C3_EXECUTION_SCHEMA === "v2_custom_code" ? CFG.C3_ORDER_TYPE : null, commandCode: CFG.C3_EXECUTION_SCHEMA === "v2_custom_code" ? redactC3V2Code(body.code) : null, entrySizeSource: action === "enter_long" && CFG.C3_EXECUTION_SCHEMA === "legacy_signal_bot" ? CFG.C3_ENTRY_SIZE_SOURCE : null, dryRun: CFG.C3_DRY_RUN });
  if (CFG.C3_PAYLOAD_AUDIT_ENABLED) {
    const auditBody = CFG.C3_EXECUTION_SCHEMA === "v2_custom_code" ? { ...body, code: redactC3V2Code(body.code) } : { ...body, secret: "REDACTED" };
    log("INFO", "C3_FORWARD_PAYLOAD_AUDIT", { requestId, action, reason, schema: CFG.C3_EXECUTION_SCHEMA, body: auditBody });
  }

  if (CFG.C3_DRY_RUN) return { ok: true, accepted: true, dryRun: true, requestId, status: 200, executionSchema: CFG.C3_EXECUTION_SCHEMA, c3Timestamp: body.timestamp || null, triggerPrice: body.trigger_price || null };
...
function statusPayload() {
  return {
    ...
    c3ExecutionContract: {
      schema: CFG.C3_EXECUTION_SCHEMA,
      ...
      entryCodeAudit: CFG.C3_EXECUTION_SCHEMA === "v2_custom_code" ? redactC3V2Code(CFG.C3_ENTER_LONG_CODE) : null,
      exitCodeAudit: CFG.C3_EXECUTION_SCHEMA === "v2_custom_code" ? redactC3V2Code(CFG.C3_EXIT_LONG_CODE) : null,
      amountPerTrade: CFG.C3_EXECUTION_SCHEMA === "v2_custom_code" ? CFG.C3_AMOUNT_PER_TRADE : null,
      amountPerTradeType: CFG.C3_EXECUTION_SCHEMA === "v2_custom_code" ? CFG.C3_AMOUNT_PER_TRADE_TYPE : null,
      orderType: CFG.C3_EXECUTION_SCHEMA === "v2_custom_code" ? CFG.C3_ORDER_TYPE : null,
      entrySizeSource: CFG.C3_EXECUTION_SCHEMA === "v2_custom_code" ? "webhook_json" : CFG.C3_ENTRY_SIZE_SOURCE,
      entryOrderIncludedInWebhook: CFG.C3_EXECUTION_SCHEMA === "v2_custom_code",
      availableBalanceFraction: CFG.C3_EXECUTION_SCHEMA === "v2_custom_code" && CFG.C3_AMOUNT_PER_TRADE_TYPE === "percents" ? CFG.C3_AMOUNT_PER_TRADE : null,
      requiredBotEntryOrder: CFG.C3_EXECUTION_SCHEMA === "v2_custom_code" ? `amountPerTrade + ${CFG.C3_AMOUNT_PER_TRADE_TYPE} + market` : "fixed quote amount + Market",
      exitOrderIncludedInWebhook: CFG.C3_EXECUTION_SCHEMA === "legacy_signal_bot" && CFG.C3_EXIT_INCLUDE_POSITION_ORDER,
      exitPercent: 100,
      nativeStopAttachedToEntry: CFG.C3_NATIVE_STOP_ENABLED,
    },
...
// ===== END SWING V1H ENGINE + C3 DYNAMIC-INSTRUMENT HOTFIX =====
  ...
  const SUPERVISOR = {
    ...
    assets: envStr("MULTI_ASSETS", "BNB")
      .split(",")
  ...
  function buildWorkerSpec(alias, index) {
    ...
    childEnv.STATE_FILE_NAME = envStr(
      ...
      envStr("STATE_FILE_NAME", "brainfvvo-swing-bnb-v1a-live-state.json")
    );
    ...
    // Standalone XRP compatibility: accept the existing global bot UUID while
    // allowing an explicit XRP_ override.
    childEnv.C3_BOT_UUID = envStr(`${alias}_C3_BOT_UUID`, envStr("C3_BOT_UUID", ""));
