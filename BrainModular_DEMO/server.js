...
const CFG = {
  ...
  // v1x: 3Commas V2 custom-code is the only supported execution transport.
  ...
  // v1p accepts the clearer PRICE_TRIGGER_* alias while preserving the older
  // PRICE_ENTRY_* variable. The alias wins when both are configured.
  PRICE_ENTRY_MAX_TRIGGER_DISTANCE_PCT: envNum("PRICE_TRIGGER_MAX_DISTANCE_PCT", envNum("PRICE_ENTRY_MAX_TRIGGER_DISTANCE_PCT", 8.0)),
  ...
  TRAILING_DIP_RECLAIM_MODE: envStr("TRAILING_DIP_RECLAIM_MODE", "live").toLowerCase(),
  TRAILING_DIP_RECLAIM_MIN_DROP_PCT: envNum("TRAILING_DIP_RECLAIM_MIN_DROP_PCT", 0.20),
  TRAILING_DIP_RECLAIM_RECLAIM_PCT: envNum("TRAILING_DIP_RECLAIM_RECLAIM_PCT", 0.10),
  TRAILING_DIP_RECLAIM_MAX_CHASE_PCT: envNum("TRAILING_DIP_RECLAIM_MAX_CHASE_PCT", 0.35),
  ...
  TRAILING_DIP_RECLAIM_REQUIRE_TICK_RECOVERY: envBool("TRAILING_DIP_RECLAIM_REQUIRE_TICK_RECOVERY", true),
  TRAILING_DIP_RECLAIM_MIN_TICK_SLOPE: envNum("TRAILING_DIP_RECLAIM_MIN_TICK_SLOPE", 0),
  TRAILING_DIP_RECLAIM_REQUIRE_RAY_NOT_BEAR: envBool("TRAILING_DIP_RECLAIM_REQUIRE_RAY_NOT_BEAR", true),
  ...
  TRAILING_DIP_RECLAIM_SEVERE_5M_VETO_ENABLED: envBool("TRAILING_DIP_RECLAIM_SEVERE_5M_VETO_ENABLED", true),
  ...
  // v1s: invalidate a breakout thesis after a material peak-to-retest collapse,
  // and require the latest completed 5m structure to recover before entry.
  ...
  BREAKOUT_RETEST_RECOVERED_5M_MAX_AGE_SEC: Math.max(1, envNum("BREAKOUT_RETEST_RECOVERED_5M_MAX_AGE_SEC", 900)),
  BREAKOUT_RETEST_RECOVERED_5M_MIN_FVVO: envNum("BREAKOUT_RETEST_RECOVERED_5M_MIN_FVVO", 0),
  BREAKOUT_RETEST_RECOVERED_5M_MIN_SLOPE: envNum("BREAKOUT_RETEST_RECOVERED_5M_MIN_SLOPE", 0),
  BREAKOUT_RETEST_RECOVERED_5M_REQUIRE_EMA_ORDER: envBool("BREAKOUT_RETEST_RECOVERED_5M_REQUIRE_EMA_ORDER", true),
  BREAKOUT_RETEST_RECOVERED_5M_REQUIRE_RAY_NOT_BEAR: envBool("BREAKOUT_RETEST_RECOVERED_5M_REQUIRE_RAY_NOT_BEAR", true),
  ...
  // v2a: bounded recovery when a real breakout pullback stops just above the
  // manually supplied retest ceiling. This is deliberately independent of the
  // wider bull-continuation observer and retains a hard anti-chase ceiling.
  BREAKOUT_NEAR_RETEST_RECOVERY_MODE: envStr("BREAKOUT_NEAR_RETEST_RECOVERY_MODE", "live").toLowerCase(),
  BREAKOUT_NEAR_RETEST_TOLERANCE_PCT: Math.max(0, envNum("BREAKOUT_NEAR_RETEST_TOLERANCE_PCT", 0.05)),
  BREAKOUT_NEAR_RETEST_MIN_PULLBACK_FROM_HIGH_PCT: Math.max(0, envNum("BREAKOUT_NEAR_RETEST_MIN_PULLBACK_FROM_HIGH_PCT", 0.08)),
  BREAKOUT_NEAR_RETEST_RECLAIM_PCT: Math.max(0, envNum("BREAKOUT_NEAR_RETEST_RECLAIM_PCT", 0.07)),
  BREAKOUT_NEAR_RETEST_MIN_ENTRY_ABOVE_CONFIRM_PCT: Math.max(0, envNum("BREAKOUT_NEAR_RETEST_MIN_ENTRY_ABOVE_CONFIRM_PCT", 0.10)),
  BREAKOUT_NEAR_RETEST_MIN_RECOVERY_OF_PULLBACK_PCT: Math.max(0, envNum("BREAKOUT_NEAR_RETEST_MIN_RECOVERY_OF_PULLBACK_PCT", 60)),
  BREAKOUT_NEAR_RETEST_MAX_ENTRY_ABOVE_CONFIRM_PCT: Math.max(0, envNum("BREAKOUT_NEAR_RETEST_MAX_ENTRY_ABOVE_CONFIRM_PCT", 0.35)),
  BREAKOUT_NEAR_RETEST_MIN_ADX: Math.max(0, envNum("BREAKOUT_NEAR_RETEST_MIN_ADX", 22)),
  BREAKOUT_NEAR_RETEST_MIN_FVVO: envNum("BREAKOUT_NEAR_RETEST_MIN_FVVO", 0.50),
  BREAKOUT_NEAR_RETEST_MIN_STRONG_SLOPE: envNum("BREAKOUT_NEAR_RETEST_MIN_STRONG_SLOPE", 0.80),
  BREAKOUT_NEAR_RETEST_MAX_RSI: envNum("BREAKOUT_NEAR_RETEST_MAX_RSI", 76),
  BREAKOUT_NEAR_RETEST_MAX_EXTENSION_FROM_EMA8_PCT: Math.max(0, envNum("BREAKOUT_NEAR_RETEST_MAX_EXTENSION_FROM_EMA8_PCT", 0.30)),
  BREAKOUT_NEAR_RETEST_CONFIRM_OBSERVATIONS: Math.max(2, Math.floor(envNum("BREAKOUT_NEAR_RETEST_CONFIRM_OBSERVATIONS", 2))),
  BREAKOUT_NEAR_RETEST_CONFIRM_MIN_SPAN_SEC: Math.max(1, envNum("BREAKOUT_NEAR_RETEST_CONFIRM_MIN_SPAN_SEC", 12)),
  BREAKOUT_NEAR_RETEST_STOP_DISTANCE_CAP_PCT: Math.max(0, envNum("BREAKOUT_NEAR_RETEST_STOP_DISTANCE_CAP_PCT", 2.50)),
  ...
  // Retained from standalone XRP v1m: the minimum lock is desired estimated
  // net profit, so configured round-trip cost is added to the gross floor.
  DYNAMIC_PROFIT_FEE_AWARE_MIN_LOCK_ENABLED: envBool("DYNAMIC_PROFIT_FEE_AWARE_MIN_LOCK_ENABLED", true),
  ...
  // Long-hold v1a: a permanent, upward-only profit ratchet. Each MFE tier selects
  // a smaller giveback window; above the final tier the last window continues.
  LONG_HOLD_RATCHET_ENABLED: envBool("LONG_HOLD_RATCHET_ENABLED", true),
  LONG_HOLD_RATCHET_SCHEDULE: envStr("LONG_HOLD_RATCHET_SCHEDULE", "1:0.65,2:0.55,3:0.45,4:0.40,5:0.35,6:0.30,7:0.25,8:0.22,9:0.20,10:0.18"),
  LONG_HOLD_MILESTONE_FLOORS: envStr("LONG_HOLD_MILESTONE_FLOORS", "1:0,2:0.25,3:0.50,4:1.00,5:2.00,6:3.00"),
  LONG_HOLD_INITIAL_SL_ENABLED: envBool("LONG_HOLD_INITIAL_SL_ENABLED", true),
  LONG_HOLD_INITIAL_SL_PCT: envNum("LONG_HOLD_INITIAL_SL_PCT", 0.80),
  LONG_HOLD_INITIAL_SL_CONFIRM_OBSERVATIONS: Math.max(1, Math.floor(envNum("LONG_HOLD_INITIAL_SL_CONFIRM_OBSERVATIONS", 2))),
  LONG_HOLD_INITIAL_SL_CONFIRM_MIN_SPAN_SEC: Math.max(0, envNum("LONG_HOLD_INITIAL_SL_CONFIRM_MIN_SPAN_SEC", 15)),
  // Emergency momentum stop: peak-to-current fall inside a short tick window.
  LONG_HOLD_FLASH_CRASH_ENABLED: envBool("LONG_HOLD_FLASH_CRASH_ENABLED", true),
  LONG_HOLD_FLASH_CRASH_WINDOW_SEC: envNum("LONG_HOLD_FLASH_CRASH_WINDOW_SEC", 15),
  ...
  RUNNER_CONTINUATION_RESCUE_MODE: envStr("RUNNER_CONTINUATION_RESCUE_MODE", "shadow").toLowerCase(),
  ...
  // v1z: bounded EMA18 hysteresis prevents a pullback being invalidated by threshold-edge noise.
  ...
  // v1s: optional late-reclaim ceiling. 0 disables the ceiling. This controls entry timing only; it never changes stops.
  ...
  REENTRY_15S_EARLY_TURN_MODE: envStr("REENTRY_15S_EARLY_TURN_MODE", "shadow").toLowerCase(),
  ...
  // v1s: when a profitable AUTO_REENTRY sees a transient tick-thesis failure while the 5m thesis remains strongly bullish,
...
function configProblems() {
  ...
  if (CFG.DYNAMIC_PROFIT_MIN_LOCK_PNL_PCT + (CFG.DYNAMIC_PROFIT_FEE_AWARE_MIN_LOCK_ENABLED ? Math.max(0, CFG.PNL_ESTIMATED_ROUND_TRIP_COST_PCT) : 0) >= CFG.DYNAMIC_PROFIT_ARM_MFE_PCT) problems.push("DYNAMIC_PROFIT_EFFECTIVE_MIN_LOCK_MUST_BE_BELOW_ARM_MFE");
  ...
  if (!["disabled", "shadow", "live"].includes(CFG.BREAKOUT_NEAR_RETEST_RECOVERY_MODE)) problems.push("INVALID_BREAKOUT_NEAR_RETEST_RECOVERY_MODE");
  if (CFG.BREAKOUT_NEAR_RETEST_TOLERANCE_PCT > 0.10 || CFG.BREAKOUT_NEAR_RETEST_RECLAIM_PCT <= 0 || CFG.BREAKOUT_NEAR_RETEST_MIN_ENTRY_ABOVE_CONFIRM_PCT > CFG.BREAKOUT_NEAR_RETEST_MAX_ENTRY_ABOVE_CONFIRM_PCT || CFG.BREAKOUT_NEAR_RETEST_MIN_RECOVERY_OF_PULLBACK_PCT > 100 || CFG.BREAKOUT_NEAR_RETEST_MAX_ENTRY_ABOVE_CONFIRM_PCT < CFG.BREAKOUT_NEAR_RETEST_RECLAIM_PCT || CFG.BREAKOUT_NEAR_RETEST_MIN_PULLBACK_FROM_HIGH_PCT <= 0 || CFG.BREAKOUT_NEAR_RETEST_MAX_RSI <= 0 || CFG.BREAKOUT_NEAR_RETEST_MAX_EXTENSION_FROM_EMA8_PCT <= 0 || CFG.BREAKOUT_NEAR_RETEST_STOP_DISTANCE_CAP_PCT <= 0) problems.push("INVALID_BREAKOUT_NEAR_RETEST_RECOVERY_THRESHOLDS");
...
function validC3V2Code(code, action, symbol = CFG.SYMBOL) {
  const value = String(code || "").trim().toUpperCase();
  const instrument = String(symbol || "").split(":").pop().replace(/[^A-Z0-9]/g, "").toUpperCase();
  if (!instrument || !value.includes(`_${instrument}_`)) return false;
  if (action === "enter_long") return value.startsWith("ENTER-LONG_");
  if (action === "exit_long") return value.startsWith("EXIT-LONG_") || value.startsWith("EXIT-ALL_");
  return false;
}

function redactC3V2Code(code) {
  const value = String(code || "").trim();
  if (!value) return null;
  return `${value.slice(0, Math.min(24, value.length))}...${value.slice(-6)}`;
}

function c3EntrySizeSource() { return "webhook_json"; }
function c3EntryOrderIncluded() { return true; }
...
function buildC3Signal(action, price, options = {}, current = nowMs()) {
  ...
  if (action === "enter_long") {
    if (!validC3V2Code(CFG.C3_ENTER_LONG_CODE, action)) throw new Error("C3_V2_ENTER_LONG_CODE_INVALID_OR_WRONG_SYMBOL");
  ...
  if (action === "exit_long") {
    if (!validC3V2Code(CFG.C3_EXIT_LONG_CODE, action)) throw new Error("C3_V2_EXIT_LONG_CODE_INVALID_OR_WRONG_SYMBOL");
    return { code: CFG.C3_EXIT_LONG_CODE };
  }
  throw new Error("C3_V2_ACTION_UNSUPPORTED");
}
...
async function forward3Commas(action, price, reason, options = {}) {
  ...
  if (CFG.C3_DRY_RUN) return { ok: true, accepted: true, dryRun: true, requestId, status: 200, executionSchema: "v2_custom_code" };
  ...
  try {
    ...
    return { ok: true, accepted: true, requestId, status: response.status, executionSchema: "v2_custom_code" };
  } catch (error) {
...
function statusPayload() {
  return {
    ...
    dynamicProfitContract: {
      ...
      minLockBasis: CFG.DYNAMIC_PROFIT_FEE_AWARE_MIN_LOCK_ENABLED ? "ESTIMATED_NET" : "SIGNAL_GROSS",
      effectiveGrossMinLockPnlPct: round(CFG.DYNAMIC_PROFIT_MIN_LOCK_PNL_PCT + (CFG.DYNAMIC_PROFIT_FEE_AWARE_MIN_LOCK_ENABLED ? Math.max(0, CFG.PNL_ESTIMATED_ROUND_TRIP_COST_PCT) : 0), 6),
    ...
    c3ExecutionContract: {
      ...
      endpoint: CFG.C3_SIGNAL_URL,
      entryCodeConfigured: Boolean(CFG.C3_ENTER_LONG_CODE),
      exitCodeConfigured: Boolean(CFG.C3_EXIT_LONG_CODE),
      entryCodeAudit: redactC3V2Code(CFG.C3_ENTER_LONG_CODE),
      exitCodeAudit: redactC3V2Code(CFG.C3_EXIT_LONG_CODE),
...
async function executeManualEntry(entry, levels, options = {}) {
  ...
  log("INFO", "FVVO_TRADE_OPEN_PENDING", { profile: PROFILE, entryPriceReference: entry, stopPrice: levels.stopPrice, profitTargetPrice: levels.profitTargetPrice || null, stopExitPercent: 100, targetExitPercent: 100, entrySizeSource: c3EntrySizeSource(), entryOrderIncludedInWebhook: c3EntryOrderIncluded(), dynamicProfitEnabled: CFG.DYNAMIC_PROFIT_EXIT_ENABLED, dynamicProfitArmMfePct: CFG.DYNAMIC_PROFIT_ARM_MFE_PCT, dynamicProfitMinLockPnlPct: CFG.DYNAMIC_PROFIT_MIN_LOCK_PNL_PCT, overheatConfirmationId: confirmation?.id || null });
  ...
  log("INFO", "FVVO_MANUAL_ONE_STOP_ENTRY_TRACKED", { entryPriceReference: entry, stopPrice: levels.stopPrice, stopDistancePct: levels.stopPct, profitTargetPrice: levels.profitTargetPrice || null, profitTargetDistancePct: levels.profitTargetPct, entrySizeSource: c3EntrySizeSource(), entryOrderIncludedInWebhook: c3EntryOrderIncluded(), nativeStopSent: CFG.C3_NATIVE_STOP_ENABLED, dynamicProfitEnabled: CFG.DYNAMIC_PROFIT_EXIT_ENABLED, dynamicProfitArmMfePct: CFG.DYNAMIC_PROFIT_ARM_MFE_PCT, requestId: result.requestId, fillVerified: false, overheatConfirmationId: confirmation?.id || null });
  return { status: 200, body: { ok: true, forwarded: true, acceptedBy3CommasWebhook: true, exchangeFillVerified: false, brainWillManageExit: true, manualEntryTracked: true, externalDealLockActive: true, profile: PROFILE, entrySizeSource: c3EntrySizeSource(), entryOrderIncludedInWebhook: c3EntryOrderIncluded(), entrySizeConfiguredInBot: false, entrySizeConfiguredInWebhook: true, entryPriceReference: entry, stopPrice: levels.stopPrice, stopDistancePct: levels.stopPct, profitTargetPrice: levels.profitTargetPrice || null, profitTargetDistancePct: levels.profitTargetPct, dynamicProfitEnabled: CFG.DYNAMIC_PROFIT_EXIT_ENABLED, dynamicProfitArmMfePct: CFG.DYNAMIC_PROFIT_ARM_MFE_PCT, dynamicProfitMinLockPnlPct: CFG.DYNAMIC_PROFIT_MIN_LOCK_PNL_PCT, requestId: result.requestId, overheatConfirmationId: confirmation?.id || null } };
}
...
function entryModeProtection(position) {
  ...
  if (!CFG.MODE_PROFIT_PROTECTION_ENABLED) return { mode, armMfePct: CFG.DYNAMIC_PROFIT_ARM_MFE_PCT, grossLockPct: null };
  ...
  return { mode, armMfePct: CFG.DYNAMIC_PROFIT_ARM_MFE_PCT, grossLockPct: null };
}

function dynamicProfitFloorPnlPct(peakPnlPct, position = null) {
  const peak = Math.max(0, finite(peakPnlPct, 0));
  if (CFG.LONG_HOLD_RATCHET_ENABLED) {
    const tiers = String(CFG.LONG_HOLD_MILESTONE_FLOORS || "").split(",").map((item) => {
      const [mfe, floor] = item.split(":").map(Number);
      return { mfe, floor };
    }).filter((x) => Number.isFinite(x.mfe) && x.mfe > 0 && Number.isFinite(x.floor) && x.floor >= 0).sort((a, b) => a.mfe - b.mfe);
    let active = null;
    for (const tier of tiers) if (peak + 1e-9 >= tier.mfe) active = tier;
    if (active) return round(active.floor, 6);
  }
  ...
  const effectiveGrossMinLock = protection.grossLockPct !== null ? protection.grossLockPct : CFG.DYNAMIC_PROFIT_MIN_LOCK_PNL_PCT + (
    CFG.DYNAMIC_PROFIT_FEE_AWARE_MIN_LOCK_ENABLED
      ? Math.max(0, CFG.PNL_ESTIMATED_ROUND_TRIP_COST_PCT)
      : 0
  );
  return round(Math.max(effectiveGrossMinLock, peak - allowedGiveback), 6);
}

function flashCrashState(position) {
  const prior = position.flashCrash && typeof position.flashCrash === "object" ? position.flashCrash : {};
  position.flashCrash = { prices: [], lastEvaluation: null, ...prior };
  if (!Array.isArray(position.flashCrash.prices)) position.flashCrash.prices = [];
  return position.flashCrash;
}

function flashCrashDecision(position, feature, price) {
  if (!CFG.LONG_HOLD_FLASH_CRASH_ENABLED || feature.kind !== CFG.FVVO_FEATURE_TICK_EVENT) return { triggered: false, reason: "FLASH_CRASH_NOT_ELIGIBLE" };
  const current = finite(feature.barTimeMs, finite(feature.receivedAtMs, nowMs()));
  const windowMs = (CFG.LONG_HOLD_FLASH_CRASH_WINDOW_SEC + CFG.LONG_HOLD_FLASH_CRASH_JITTER_SEC) * 1000;
  const minSpanMs = CFG.LONG_HOLD_FLASH_CRASH_MIN_SPAN_SEC * 1000;
  const s = flashCrashState(position);
  s.prices = s.prices.filter((x) => current - finite(x.atMs, current) <= windowMs && current >= finite(x.atMs, current));
  const references = s.prices.filter((x) => current - x.atMs >= minSpanMs);
  const peakPoint = references.reduce((best, x) => !best || x.price > best.price ? x : best, null);
  const dropPct = peakPoint ? ((peakPoint.price - price) / peakPoint.price) * 100 : 0;
  s.prices.push({ atMs: current, price: round(price, 8) });
  s.lastEvaluation = { atMs: current, price: round(price, 8), referencePrice: peakPoint?.price || null, spanSec: peakPoint ? (current - peakPoint.atMs) / 1000 : 0, dropPct: round(dropPct, 6) };
  return { triggered: Boolean(peakPoint && dropPct + 1e-9 >= CFG.LONG_HOLD_FLASH_CRASH_DROP_PCT), reason: "FLASH_CRASH_ROLLING_PEAK_DROP", ...s.lastEvaluation };
}

function initialLossStopDecision(position, feature, price) {
  if (!CFG.LONG_HOLD_INITIAL_SL_ENABLED || feature.kind !== CFG.FVVO_FEATURE_TICK_EVENT) return { confirmed: false, reason: "INITIAL_SL_NOT_ELIGIBLE" };
  const thresholdPrice = position.entryPriceReference * (1 - CFG.LONG_HOLD_INITIAL_SL_PCT / 100);
  const s = position.initialLossStop = { breachAtMs: 0, observations: 0, ...(position.initialLossStop || {}) };
  if (price > thresholdPrice + 1e-9) {
    if (s.observations) position.initialLossStop = { breachAtMs: 0, observations: 0 };
    return { confirmed: false, reason: "ABOVE_INITIAL_SL", thresholdPrice: round(thresholdPrice, 8) };
  }
  const current = finite(feature.receivedAtMs, nowMs());
  if (!s.breachAtMs) { s.breachAtMs = current; s.observations = 1; } else s.observations = Number(s.observations || 0) + 1;
  const elapsedSec = (current - s.breachAtMs) / 1000;
  return { confirmed: s.observations >= CFG.LONG_HOLD_INITIAL_SL_CONFIRM_OBSERVATIONS && elapsedSec >= CFG.LONG_HOLD_INITIAL_SL_CONFIRM_MIN_SPAN_SEC, reason: "INITIAL_SL_CONFIRM", thresholdPrice: round(thresholdPrice, 8), observations: s.observations, elapsedSec };
}
...
function dynamicFloorBreakConfirmed(position, markPrice, pnlPct) {
  const d = dynamicProfitState(position);
  if (!CFG.DYNAMIC_PROFIT_EXIT_ENABLED || !d.armed || finite(d.protectedPnlPct, -1) < 0) return { confirmed: false, reason: "DYNAMIC_PROFIT_NOT_ARMED" };
...
function modeStructuralExitFailureConfirmed(position, feature, price, pnlPct) {
  const mode = String(position?.priceTrigger?.mode || "").toLowerCase();
  const supported = ["dip", "trailing_dip_reclaim", "trailing_dip_reclaim_zone", "hybrid_pullback_reclaim_zone"].includes(mode);
  if (!CFG.MODE_STRUCTURAL_EXIT_ENABLED || !supported || feature.kind !== CFG.FVVO_FEATURE_TICK_EVENT) return { confirmed: false, reason: "MODE_STRUCTURAL_EXIT_NOT_ELIGIBLE" };
  const s = position.modeStructuralExit = position.modeStructuralExit && typeof position.modeStructuralExit === "object" ? position.modeStructuralExit : { breachAtMs: 0, observations: 0 };
  const low = finite(position?.priceTrigger?.observedLowPrice, null);
  let threshold = null;
  if (mode === "dip") threshold = position.entryPriceReference * (1 - CFG.MODE_STRUCTURAL_DIP_MAX_LOSS_PCT / 100);
  else if (low !== null) {
    ...
    threshold = low * (1 - buffer / 100);
  }
  if (!(threshold > 0) || price > threshold + 1e-9) {
    if (s.observations) position.modeStructuralExit = { breachAtMs: 0, observations: 0 };
    return { confirmed: false, reason: threshold ? "ABOVE_MODE_STRUCTURAL_FLOOR" : "NO_ENTRY_STRUCTURAL_LOW", mode, threshold };
  }
  const current = nowMs();
  if (!s.breachAtMs) { s.breachAtMs = current; s.observations = 1; } else s.observations = Number(s.observations || 0) + 1;
  s.lastBreachPrice = price;
  const elapsedSec = (current - s.breachAtMs) / 1000;
  return { confirmed: s.observations >= CFG.MODE_STRUCTURAL_EXIT_CONFIRM_OBSERVATIONS && elapsedSec >= CFG.MODE_STRUCTURAL_EXIT_CONFIRM_MIN_SPAN_SEC, reason: "MODE_STRUCTURAL_FLOOR_CONFIRM", mode, threshold: round(threshold, 8), observations: s.observations, elapsedSec, pnlPct };
}
...
async function manageExit(feature) {
  ...
  // Emergency price-velocity protection has first priority after a valid mark.
  const flashCrash = flashCrashDecision(p, feature, price);
  if (flashCrash.triggered) {
    log("ERROR", "FVVO_LONG_HOLD_FLASH_CRASH_EXIT", { price, latestPnlPct: round(pnl, 6), thresholdDropPct: CFG.LONG_HOLD_FLASH_CRASH_DROP_PCT, ...flashCrash });
    await persistState(`long_hold_flash_crash_${feature.kind}`);
    await requestFullExit("FVVO_LONG_HOLD_FLASH_CRASH_15S", price, feature.kind);
    return;
  }

  const initialSl = initialLossStopDecision(p, feature, price);
  if (initialSl.confirmed) {
    log("ERROR", "FVVO_LONG_HOLD_INITIAL_SL_EXIT", { price, latestPnlPct: round(pnl, 6), stopLossPct: CFG.LONG_HOLD_INITIAL_SL_PCT, ...initialSl });
    await persistState(`long_hold_initial_sl_${feature.kind}`);
    await requestFullExit("FVVO_LONG_HOLD_INITIAL_SL", price, feature.kind);
    return;
  }
  ...
  if (dynamicUpdate.armedNow) {
    log("INFO", "FVVO_DYNAMIC_PROFIT_ARMED", { entryPrice: p.entryPriceReference, entryMode: p.priceTrigger?.mode || null, armMfePct: entryModeProtection(p).armMfePct, armObservedPnlPct: d.armedAtPnlPct, protectedPnlPct: d.protectedPnlPct, protectedPrice: d.protectedPrice, price });
  }
  ...
  if (modeStructural.confirmed) {
    ...
    await requestFullExit(`FVVO_MODE_STRUCTURAL_EXIT_${String(modeStructural.mode || "UNKNOWN").toUpperCase()}`, price, feature.kind);
    return;
  }
  ...
  // v1s: an already-armed re-entry continuation grace never bypasses the manual stop, dynamic floor, or runner trail above.
...
async function evaluateReentryShadow(feature) {
  ...
  log("INFO", "FVVO_AUTO_REENTRY_ENTRY_TRACKED", { candidateId: candidate.id, reentryNumber: candidate.sequence, entryPriceReference: price, stopPrice: projectedStop.stopPrice, stopDistancePct: projectedStop.stopDistancePct, requestId: result.requestId, entrySizeSource: c3EntrySizeSource(), entryOrderIncludedInWebhook: c3EntryOrderIncluded(), fillVerified: false });
}
...
function priceEntryStatusPayload() {
  ...
  return {
    ...
    trailingDipReclaim: {
      ...
      minTickFvvo: CFG.TRAILING_DIP_RECLAIM_MIN_TICK_FVVO,
      confirmObservations: CFG.TRAILING_DIP_RECLAIM_CONFIRM_OBSERVATIONS,
      confirmMinSpanSec: CFG.TRAILING_DIP_RECLAIM_CONFIRM_MIN_SPAN_SEC,
    },
    ...
    breakoutRetestReclaimZone: {
      ...
      nearRetestRecovery: {
        mode: breakoutNearRetestRecoveryMode(),
        tolerancePct: CFG.BREAKOUT_NEAR_RETEST_TOLERANCE_PCT,
        minPullbackFromHighPct: CFG.BREAKOUT_NEAR_RETEST_MIN_PULLBACK_FROM_HIGH_PCT,
        reclaimPct: CFG.BREAKOUT_NEAR_RETEST_RECLAIM_PCT,
        minEntryAboveConfirmPct: CFG.BREAKOUT_NEAR_RETEST_MIN_ENTRY_ABOVE_CONFIRM_PCT,
        minRecoveryOfPullbackPct: CFG.BREAKOUT_NEAR_RETEST_MIN_RECOVERY_OF_PULLBACK_PCT,
        maxEntryAboveConfirmPct: CFG.BREAKOUT_NEAR_RETEST_MAX_ENTRY_ABOVE_CONFIRM_PCT,
        minAdx: CFG.BREAKOUT_NEAR_RETEST_MIN_ADX,
        minFvvo: CFG.BREAKOUT_NEAR_RETEST_MIN_FVVO,
        minStrongSlope: CFG.BREAKOUT_NEAR_RETEST_MIN_STRONG_SLOPE,
        maxRsi: CFG.BREAKOUT_NEAR_RETEST_MAX_RSI,
        maxExtensionFromEma8Pct: CFG.BREAKOUT_NEAR_RETEST_MAX_EXTENSION_FROM_EMA8_PCT,
...
function trailingTickRecoveryOk(feature) {
  ...
  const slope = finite(feature.slope, null);
  const fvvo = finite(feature.fvvo, null);
  if (ema8 === null || feature.price < ema8 || slope === null || slope < CFG.TRAILING_DIP_RECLAIM_MIN_TICK_SLOPE || fvvo === null || fvvo < CFG.TRAILING_DIP_RECLAIM_MIN_TICK_FVVO || feature.redPulse === true) return false;
...
function trailingDipSevere5mVeto() {
  if (!CFG.TRAILING_DIP_RECLAIM_SEVERE_5M_VETO_ENABLED) return { veto: false, reason: "DISABLED" };
  const ctx = state.lastFeature5m;
  if (!ctx || ageSec(ctx) > CFG.ENTRY_5M_BEAR_GUARD_MAX_AGE_SEC) return { veto: false, reason: "NO_FRESH_5M" };
  const close = finite(ctx.close, ctx.price), ema8 = finite(ctx.ema8, null), ema18 = finite(ctx.ema18, null), fvvo = finite(ctx.fvvo, null), slope = finite(ctx.slope, null);
  const rayBear = String(ctx.rayRegime || "").toUpperCase().startsWith("RAY_BEAR");
  const veto = close !== null && ema8 !== null && ema18 !== null && close < ema8 && ema8 < ema18 && fvvo !== null && fvvo <= CFG.TRAILING_DIP_RECLAIM_SEVERE_5M_MAX_FVVO && slope !== null && slope <= CFG.TRAILING_DIP_RECLAIM_SEVERE_5M_MAX_SLOPE && rayBear;
  return { veto, reason: veto ? "SEVERE_COMPLETED_5M_BREAKDOWN" : "NO_SEVERE_COMPLETED_5M_BREAKDOWN", close, ema8, ema18, fvvo, slope, rayRegime: ctx.rayRegime || null };
}
...
function breakoutRetestRecovered5mEvidence() {
  const ctx = state.lastFeature5m;
  if (!ctx) return { qualifies: false, reason: "NO_COMPLETED_5M_CONTEXT" };
  ...
  const evidence = {
    ...
    fvvo,
    slope,
    rayRegime: ctx.rayRegime || null,
    barTimeMs: finite(ctx.barTimeMs, 0),
    ageSec: round(age, 3),
    fresh: age <= CFG.BREAKOUT_RETEST_RECOVERED_5M_MAX_AGE_SEC,
    closeAboveEma8: close !== null && ema8 !== null && close >= ema8,
    emaOrderOk: !CFG.BREAKOUT_RETEST_RECOVERED_5M_REQUIRE_EMA_ORDER || (ema8 !== null && ema18 !== null && ema8 >= ema18),
    fvvoOk: fvvo !== null && fvvo >= CFG.BREAKOUT_RETEST_RECOVERED_5M_MIN_FVVO,
    slopeOk: slope !== null && slope >= CFG.BREAKOUT_RETEST_RECOVERED_5M_MIN_SLOPE,
    rayOk: !CFG.BREAKOUT_RETEST_RECOVERED_5M_REQUIRE_RAY_NOT_BEAR || nonBearRay(ctx.rayRegime),
  };
  evidence.qualifies = evidence.fresh && evidence.closeAboveEma8 && evidence.emaOrderOk && evidence.fvvoOk && evidence.slopeOk && evidence.rayOk;
  evidence.reason = evidence.qualifies ? "RECOVERED_5M_CONFIRMED" : "RECOVERED_5M_NOT_READY";
  return evidence;
}

function breakoutNearRetestRecoveryMode() { return CFG.BREAKOUT_NEAR_RETEST_RECOVERY_MODE; }

function breakoutNearRetestRecoveryEvidence(feature) {
  const ema8 = finite(feature.ema8, null), ema18 = finite(feature.ema18, null);
  const rsi = finite(feature.rsi, null), adx = finite(feature.adx, null);
  ...
  const evidence = {
    aboveEma8: ema8 !== null && feature.price >= ema8,
    ema8AboveEma18: ema8 !== null && ema18 !== null && ema8 > ema18,
    rsiOk: rsi !== null && rsi <= CFG.BREAKOUT_NEAR_RETEST_MAX_RSI,
    adxOk: adx !== null && adx >= CFG.BREAKOUT_NEAR_RETEST_MIN_ADX,
    fvvoOk: fvvo !== null && fvvo >= CFG.BREAKOUT_NEAR_RETEST_MIN_FVVO,
    strongSlopeOk: slope !== null && slope >= CFG.BREAKOUT_NEAR_RETEST_MIN_STRONG_SLOPE,
    rayBullOk: String(feature.rayRegime || "").toUpperCase() === "RAY_BULL",
    crossUp: Boolean(feature.crossUp),
    extensionFromEma8Pct: round(extensionFromEma8Pct, 6),
    emaExtensionOk: extensionFromEma8Pct <= CFG.BREAKOUT_NEAR_RETEST_MAX_EXTENSION_FROM_EMA8_PCT + 1e-9,
    fvvo, slope, adx, rsi,
  };
  evidence.ok = evidence.aboveEma8 && evidence.ema8AboveEma18 && evidence.rsiOk && evidence.adxOk && evidence.fvvoOk && evidence.strongSlopeOk && evidence.rayBullOk && evidence.emaExtensionOk;
  return evidence;
}

async function evaluateBreakoutNearRetestRecovery(pending, feature, rangeHigh, breakoutConfirmPrice) {
  if (breakoutNearRetestRecoveryMode() === "disabled") return false;
  const t = pending.trailing || (pending.trailing = {}), current = nowMs();
  const breakoutAtMs = finite(t.breakoutAtMs, 0);
  if (!(breakoutAtMs > 0) || current > finite(t.trackingExpiresAtMs, 0) || !(feature.price > rangeHigh + 1e-9)) return false;

  const highest = Math.max(finite(t.highestBreakoutPrice, feature.price), feature.price);
  const nearRetestCeiling = rangeHigh * (1 + CFG.BREAKOUT_NEAR_RETEST_TOLERANCE_PCT / 100);
  const pullbackFromHighPct = percentageBelow(highest, feature.price);
  if (feature.price <= nearRetestCeiling + 1e-9 && pullbackFromHighPct + 1e-9 >= CFG.BREAKOUT_NEAR_RETEST_MIN_PULLBACK_FROM_HIGH_PCT) {
    const priorLow = finite(t.nearRetestLowPrice, null);
    if (priorLow === null || feature.price < priorLow - 1e-9) {
      t.nearRetestLowPrice = round(feature.price, 8);
      t.nearRetestLowAtMs = feature.receivedAtMs || current;
      t.nearRetestRecoveryObservations = 0;
      t.nearRetestFirstRecoveryAtMs = 0;
      t.nearRetestLastEvidence = null;
      t.nearRetestShadowCandidateLowPrice = null;
      log("INFO", "FVVO_BREAKOUT_NEAR_RETEST_LOW_QUALIFIED", { triggerId: pending.id, breakoutConfirmPrice, retestRangeHigh: rangeHigh, nearRetestCeiling: round(nearRetestCeiling, 8), nearRetestLowPrice: t.nearRetestLowPrice, missAboveRangeHighPct: round(percentPnl(rangeHigh, feature.price), 6), pullbackFromHighPct: round(pullbackFromHighPct, 6) });
    }
  }

  const low = finite(t.nearRetestLowPrice, null);
  if (low === null) return false;
  const reclaimTarget = low * (1 + CFG.BREAKOUT_NEAR_RETEST_RECLAIM_PCT / 100);
  const minConfirmPrice = breakoutConfirmPrice * (1 + CFG.BREAKOUT_NEAR_RETEST_MIN_ENTRY_ABOVE_CONFIRM_PCT / 100);
  const maxEntryPrice = breakoutConfirmPrice * (1 + CFG.BREAKOUT_NEAR_RETEST_MAX_ENTRY_ABOVE_CONFIRM_PCT / 100);
  const pullbackRange = Math.max(0, highest - low);
  const recoveredPullbackPct = pullbackRange > 0 ? ((feature.price - low) / pullbackRange) * 100 : 0;
  if (feature.price + 1e-9 < reclaimTarget || feature.price + 1e-9 < minConfirmPrice || feature.price > maxEntryPrice + 1e-9 || recoveredPullbackPct + 1e-9 < CFG.BREAKOUT_NEAR_RETEST_MIN_RECOVERY_OF_PULLBACK_PCT) return false;

  const evidence = breakoutNearRetestRecoveryEvidence(feature);
  if (!evidence.ok) return false;
  const previous = t.nearRetestLastEvidence;
  const improving = !previous || (feature.price >= previous.price - 1e-9 && evidence.fvvo >= previous.fvvo - 1e-9 && evidence.slope >= previous.slope - 1e-9);
  if (!improving) {
    t.nearRetestRecoveryObservations = 0;
    t.nearRetestFirstRecoveryAtMs = 0;
  }
  t.nearRetestLastEvidence = { price: feature.price, fvvo: evidence.fvvo, slope: evidence.slope, atMs: feature.receivedAtMs || current };
  if (improving) {
    t.nearRetestRecoveryObservations = Math.floor(finite(t.nearRetestRecoveryObservations, 0)) + 1;
    if (!finite(t.nearRetestFirstRecoveryAtMs, 0)) t.nearRetestFirstRecoveryAtMs = feature.receivedAtMs || current;
  }
  const observations = Math.floor(finite(t.nearRetestRecoveryObservations, 0));
  const spanSec = Math.max(0, ((feature.receivedAtMs || current) - finite(t.nearRetestFirstRecoveryAtMs, feature.receivedAtMs || current)) / 1000);
  const confirmed = evidence.crossUp || (observations >= CFG.BREAKOUT_NEAR_RETEST_CONFIRM_OBSERVATIONS && spanSec + 1e-9 >= CFG.BREAKOUT_NEAR_RETEST_CONFIRM_MIN_SPAN_SEC);
  if (!confirmed) return false;

  const checked = validateStoredPriceTriggerAtExecution(pending, feature.price);
  if (!checked.ok || checked.levels.stopPct > CFG.BREAKOUT_NEAR_RETEST_STOP_DISTANCE_CAP_PCT + 1e-9) return false;
  const candidate = { triggerId: pending.id, breakoutConfirmPrice, retestRangeHigh: rangeHigh, nearRetestCeiling: round(nearRetestCeiling, 8), nearRetestLowPrice: low, missAboveRangeHighPct: round(percentPnl(rangeHigh, low), 6), reclaimTargetPrice: round(reclaimTarget, 8), minConfirmPrice: round(minConfirmPrice, 8), maxEntryPrice: round(maxEntryPrice, 8), recoveredPullbackPct: round(recoveredPullbackPct, 6), candidateEntryPrice: feature.price, stopPrice: pending.stopPrice, stopDistancePct: checked.levels.stopPct, observations, spanSec: round(spanSec, 3), confirmation: evidence.crossUp ? "CROSS_UP" : "TWO_IMPROVING_OBSERVATIONS", recovery: evidence };
  if (breakoutNearRetestRecoveryMode() === "shadow") {
    if (finite(t.nearRetestShadowCandidateLowPrice, null) !== low) {
      t.nearRetestShadowCandidateLowPrice = low;
      log("INFO", "FVVO_BREAKOUT_NEAR_RETEST_RECOVERY_SHADOW_CANDIDATE", { ...candidate, action: "NO_ORDER_SHADOW_ONLY" });
    }
    return false;
  }
  log("INFO", "FVVO_BREAKOUT_NEAR_RETEST_RECOVERY_CONFIRMED", candidate);
  await enterFromPriceTrigger(pending, feature, "BREAKOUT_NEAR_RETEST_RECOVERY_CONFIRMED");
  return true;
}
...
function validateCampaignArm(body, validated, active) {
  ...
  if (["preferred", "deep_alternative"].includes(entryRole) && !["trailing_dip_reclaim", "trailing_dip_reclaim_zone", "confirmed_pullback_reclaim_zone", "hybrid_pullback_reclaim_zone"].includes(validated.triggerMode)) return { ok: false, error: "PREFERRED_AND_DEEP_ROLES_REQUIRE_TRAILING_CONFIRMED_OR_HYBRID_PULLBACK_RECLAIM" };
  ...
  if (["preferred", "deep_alternative"].includes(entryRole)) {
    ...
    if (otherDip) {
      const newLow = Number(validated.activationRangeLow ?? validated.activationPrice);
      const newHigh = Number(validated.activationRangeHigh ?? validated.activationPrice);
      const oldLow = Number(otherDip.activationRangeLow ?? otherDip.activationPrice);
      const oldHigh = Number(otherDip.activationRangeHigh ?? otherDip.activationPrice);
...
function entryGateSanitizedSetup(body) {
  const copy = clone(body || {});
  delete copy.secret;
  delete copy.confirmation_token;
  delete copy.confirmationToken;
  copy.action = "arm_price_entry";
  return copy;
}

async function armPriceEntry(body) {
  if (!CFG.ENTRY_GATE_CONFIRM_REQUIRED) return armPriceEntryConfirmed(body);
  const issue = configProblems()[0];
  if (issue) return { status: 503, body: { ok: false, error: issue } };
  ...
  if (stateBlocksNewEntry()) return { status: 409, body: { ok: false, error: stateBlocksNewEntry(), status: statusPayload() } };
  if (CFG.MANUAL_REQUIRE_FRESH_FEATURE_TICK && !isFeatureFresh()) return { status: 409, body: { ok: false, error: "FRESH_FEATURE_TICK_REQUIRED", featureAgeSec: ageSec(state.lastFeature) } };
  const validated = validatePriceTriggerCommand(body, finite(state.lastFeature?.price, null));
  if (!validated.ok) return { status: 400, body: { ok: false, error: validated.error } };
  const campaign = validateCampaignArm(body, validated, activePriceEntryItems());
  if (!campaign.ok) return { status: 409, body: { ok: false, error: campaign.error, priceTriggerEntry: priceEntryStatusPayload() } };
  const assessment = entryGate.assess({ audit: state.audit, fastFeature: state.lastFeature, role: campaign.entryRole });
  const token = crypto.randomUUID();
  const expiresAtMs = nowMs() + CFG.ENTRY_GATE_CONFIRM_TTL_SEC * 1000;
...
async function confirmPriceEntryGate(body) {
  const preview = state.manual?.entryGatePreview;
  if (!preview) return { status: 409, body: { ok: false, error: "NO_ENTRY_GATE_PREVIEW" } };
  if (nowMs() > finite(preview.expiresAtMs, 0)) { state.manual.entryGatePreview = null; await persistState("entry_gate_preview_expired"); return { status: 410, body: { ok: false, error: "ENTRY_GATE_CONFIRMATION_EXPIRED" } }; }
  const token = String(body.confirmation_token || body.confirmationToken || "");
  if (!safeTimingEqual(preview.token, token)) return { status: 403, body: { ok: false, error: "BAD_ENTRY_GATE_CONFIRMATION_TOKEN" } };
  const setup = preview.setup;
  const assessment = preview.assessment;
  state.manual.entryGatePreview = null;
  log("INFO", "FVVO_ENTRY_GATE_CONFIRMED", { verdict: assessment.verdict, score: assessment.score, confidence: assessment.confidence, action: "ARM_SETUP" });
  return armPriceEntryConfirmed(setup);
}

async function declinePriceEntryGate(body) {
  const preview = state.manual?.entryGatePreview;
  if (!preview) return { status: 200, body: { ok: true, setupAborted: true, alreadyAbsent: true } };
  const token = String(body.confirmation_token || body.confirmationToken || "");
  if (token && !safeTimingEqual(preview.token, token)) return { status: 403, body: { ok: false, error: "BAD_ENTRY_GATE_CONFIRMATION_TOKEN" } };
  state.manual.entryGatePreview = null;
  await persistState("entry_gate_declined");
  log("WARN", "FVVO_ENTRY_GATE_DECLINED", { verdict: preview.assessment?.verdict, score: preview.assessment?.score, reason: String(body.reason || "user_selected_no"), setupArmed: false });
  log("WARN", "FVVO_SETUP_ABORTED_BY_USER", { reason: String(body.reason || "user_selected_no") });
  return { status: 200, body: { ok: true, setupAborted: true, setupArmed: false } };
}

async function armPriceEntryConfirmed(body) {
  const issue = configProblems()[0];
  if (issue) return { status: 503, body: { ok: false, error: issue } };
  if (!CFG.PRICE_ENTRY_ENABLED || !CFG.MANUAL_ALLOW_ARM_PRICE_ENTRY) return { status: 403, body: { ok: false, error: "PRICE_TRIGGER_ENTRY_DISABLED" } };
  if (String(body.profile || CFG.MANUAL_ENTRY_DEFAULT_PROFILE).trim().toUpperCase() !== PROFILE) return { status: 400, body: { ok: false, error: "ONLY_SWING_BALANCED_STRUCTURE_EXIT_PROFILE_ALLOWED" } };
  ...
  return { status: 200, body: { ok: true, priceEntryArmed: true, orderTypeOnTrigger: (isTrailing || pending.triggerMode === "breakout_retest_reclaim_zone") ? "market_on_reclaim" : "market", entrySizeSource: c3EntrySizeSource(), entryOrderIncludedInWebhook: c3EntryOrderIncluded(), trigger: priceEntryStatusPayload().pendingList.find((item) => item.id === pending.id) || priceEntryStatusPayload().pending } };
}
...
async function cancelPriceEntry(body) {
  ...
  for (const item of cancelled) {
    ...
    if (item.entryCampaign && item.triggerMode === "trailing_dip_reclaim") log("WARN", "FVVO_CAMPAIGN_TRAILING_DIP_RECLAIM_CANCELLED", { entryCampaign: item.entryCampaign, entryRole: item.entryRole, triggerId: item.id, reason: item.cancelReason });
  }
...
async function enterFromPriceTrigger(pending, feature, modeReason) {
  ...
  for (const item of siblingCancelled) {
    ...
    if (item.entryCampaign && item.triggerMode === "trailing_dip_reclaim") log("WARN", "FVVO_CAMPAIGN_TRAILING_DIP_RECLAIM_CANCELLED", { entryCampaign: item.entryCampaign, entryRole: item.entryRole, triggerId: item.id, reason: item.resolutionReason, winningEntryRole: consumed.entryRole, winningTriggerId: consumed.id });
  }
  ...
  state.position.priceTrigger = { id: consumed.id, mode: consumed.triggerMode, entryCampaign: consumed.entryCampaign || null, entryRole: consumed.entryRole || "standalone", campaignOrdinal: Math.max(0, Math.floor(finite(consumed.campaignOrdinal, 0))), price: consumed.triggerPrice, activationPrice: consumed.activationPrice || null, activationRangeLow: consumed.activationRangeLow || null, activationRangeHigh: consumed.activationRangeHigh || null, breakoutConfirmPrice: consumed.breakoutConfirmPrice || null, retestRangeLow: consumed.retestRangeLow || null, retestRangeHigh: consumed.retestRangeHigh || null, observedLowPrice: finite(consumed.trailing?.observedLowPrice, null), observedLowAtMs: finite(consumed.trailing?.observedLowAtMs, 0), armedReferencePrice: consumed.armedReferencePrice, triggeredAt: consumed.triggeredAt };
...
async function evaluateTrailingDipReclaim(pending, previousPrice, feature) {
  ...
  if (t.phase === "WATCHING_DIP" || t.phase === "TRACKING_RECLAIM") {
    ...
    if (!(t.fastConfirmStartedAtMs > 0)) t.fastConfirmStartedAtMs = current;
    t.fastConfirmObservations = Math.max(0, Math.floor(finite(t.fastConfirmObservations, 0))) + 1;
    const confirmSpanSec = (current - t.fastConfirmStartedAtMs) / 1000;
    if (t.fastConfirmObservations < CFG.TRAILING_DIP_RECLAIM_CONFIRM_OBSERVATIONS || confirmSpanSec + 1e-9 < CFG.TRAILING_DIP_RECLAIM_CONFIRM_MIN_SPAN_SEC) {
      await persistState("trailing_dip_reclaim_fast_confirming");
      log("INFO", "FVVO_TRAILING_DIP_RECLAIM_FAST_CONFIRMING", { triggerId: pending.id, observedLowPrice: low, reclaimTargetPrice: t.reclaimTargetPrice, executionPrice: feature.price, observations: t.fastConfirmObservations, requiredObservations: CFG.TRAILING_DIP_RECLAIM_CONFIRM_OBSERVATIONS, spanSec: round(confirmSpanSec, 3), minSpanSec: CFG.TRAILING_DIP_RECLAIM_CONFIRM_MIN_SPAN_SEC, fvvo: finite(feature.fvvo, null), slope: finite(feature.slope, null), rayRegime: feature.rayRegime || null });
      return;
    }
    const severe5m = trailingDipSevere5mVeto();
    if (severe5m.veto) {
      t.fastConfirmObservations = 0; t.fastConfirmStartedAtMs = 0;
      await persistState("trailing_dip_reclaim_severe_5m_veto");
      log("WARN", "FVVO_TRAILING_DIP_RECLAIM_SEVERE_5M_VETO", { triggerId: pending.id, executionPrice: feature.price, observedLowPrice: low, context5m: severe5m, action: "WAIT_FAST_RECOVERY_WITHOUT_5M_RELEASE_DELAY" });
      return;
    }
    const checked = validateStoredPriceTriggerAtExecution(pending, feature.price);
    if (!checked.ok) {
...
async function evaluateHybridPullbackReclaimZone(pending, previousPrice, feature) {
  ...
  if (totalReboundPct > CFG.HYBRID_PULLBACK_MAX_REBOUND_FROM_LOW_PCT + 1e-9 || recoveryAgeSec > CFG.HYBRID_PULLBACK_MAX_RECOVERY_AGE_SEC) {
    const cancelled = resolvePriceEntryPending("CANCELLED", totalReboundPct > CFG.HYBRID_PULLBACK_MAX_REBOUND_FROM_LOW_PCT ? "HYBRID_PULLBACK_REBOUND_CHASE_TOO_LARGE" : "HYBRID_PULLBACK_RECOVERY_TOO_OLD", { trailing: t }, pending);
    await persistState("hybrid_pullback_anti_chase_cancelled");
    log("WARN", "FVVO_HYBRID_PULLBACK_CANCELLED", { triggerId: cancelled?.id || pending.id, reason: cancelled?.resolutionReason, observedLowPrice: low, executionPrice: feature.price, reboundPct: round(totalReboundPct, 6), maxReboundPct: CFG.HYBRID_PULLBACK_MAX_REBOUND_FROM_LOW_PCT, recoveryAgeSec: round(recoveryAgeSec, 3), maxRecoveryAgeSec: CFG.HYBRID_PULLBACK_MAX_RECOVERY_AGE_SEC });
    return;
  }
...
async function evaluateBreakoutRetestReclaimZone(pending, previousPrice, feature) {
  ...
  if (t.phase === "WAITING_RETEST" || t.phase === "TRACKING_RECLAIM") {
    ...
    if (t.phase === "WAITING_RETEST") {
      if (feature.price > rangeHigh + 1e-9) {
        const nearRetestEntered = await evaluateBreakoutNearRetestRecovery(pending, feature, rangeHigh, breakoutConfirmPrice);
        if (nearRetestEntered) return;
    ...
    const low = finite(t.retestLowPrice, feature.price);
    const lowStopBufferPct = low > pending.stopPrice ? percentageBelow(low, pending.stopPrice) : 0;
    const highestBreakoutPrice = finite(t.highestBreakoutPrice, breakoutConfirmPrice);
    const peakPullbackPct = highestBreakoutPrice > 0 ? percentageBelow(highestBreakoutPrice, low) : 0;
    t.retestPenetrationPct = round(percentageBelow(rangeHigh, low), 6);
    t.lowStopBufferPct = round(lowStopBufferPct, 6);
    t.peakPullbackPct = round(peakPullbackPct, 6);
    if (CFG.BREAKOUT_RETEST_MAX_PEAK_PULLBACK_PCT > 0 && peakPullbackPct + 1e-9 >= CFG.BREAKOUT_RETEST_MAX_PEAK_PULLBACK_PCT) {
      ...
      log("WARN", "FVVO_BREAKOUT_RETEST_RECLAIM_ZONE_CANCELLED", { triggerId: cancelled.id, reason: cancelled.resolutionReason, breakoutConfirmPrice, highestBreakoutPrice, retestLowPrice: low, peakPullbackPct: round(peakPullbackPct, 6), maxPeakPullbackPct: CFG.BREAKOUT_RETEST_MAX_PEAK_PULLBACK_PCT, executionPrice: feature.price, action: "INVALIDATE_FAILED_BREAKOUT" });
      return;
    }
    ...
    if (CFG.BREAKOUT_RETEST_REQUIRE_RECOVERED_5M) {
      const recovered5m = breakoutRetestRecovered5mEvidence();
      t.lastRecovered5mEvidence = recovered5m;
      if (!recovered5m.qualifies) {
        await persistState("breakout_retest_wait_recovered_5m");
        log("INFO", "FVVO_BREAKOUT_RETEST_RECLAIM_ZONE_WAIT_RECOVERED_5M", { triggerId: pending.id, breakoutConfirmPrice, retestRangeLow: rangeLow, retestRangeHigh: rangeHigh, retestLowPrice: low, reclaimTargetPrice: t.reclaimTargetPrice, executionPrice: feature.price, recovered5m });
        return;
      }
    }
    const checked = validateStoredPriceTriggerAtExecution(pending, feature.price);
...
async function handleManual(body) {
  ...
  if (action === "confirm_price_entry_gate") return confirmPriceEntryGate(body);
  if (action === "decline_price_entry_gate") return declinePriceEntryGate(body);
...
async function start() {
  ...
  try {
    ...
    log("INFO", "C3_V2_EXECUTION_ROUTE_AUDIT", {
      schema: "v2_custom_code",
      endpoint: CFG.C3_SIGNAL_URL,
      symbol: CFG.SYMBOL,
      tvExchange: c3MarketAudit.tvExchange,
      tvInstrument: c3MarketAudit.tvInstrument,
      entryCode: redactC3V2Code(CFG.C3_ENTER_LONG_CODE),
      exitCode: redactC3V2Code(CFG.C3_EXIT_LONG_CODE),
      amountPerTrade: CFG.C3_AMOUNT_PER_TRADE,
      amountPerTradeType: CFG.C3_AMOUNT_PER_TRADE_TYPE,
      orderType: CFG.C3_ORDER_TYPE,
    });
...
// ===== END SWING V1H ENGINE + C3 DYNAMIC-INSTRUMENT HOTFIX =====
  ...
  const SUPERVISOR = {
    brain: envStr("MULTI_BRAIN_NAME", "BrainFVVO_Swing_XRP_v2b_ENTRY_GATE_CONFIRM_LIVE"),
    ...
    assets: envStr("MULTI_ASSETS", "XRP")
      .split(",")
  ...
  function buildWorkerSpec(alias, index) {
    ...
    const symbol = envStr(`${alias}_SYMBOL`, envStr("SYMBOL", `BINANCE:${alias}USDT`)).toUpperCase();
    ...
    childEnv.BRAIN_NAME = envStr(
      `${alias}_BRAIN_NAME`,
      envStr("BRAIN_NAME", "BrainFVVO_Swing_XRP_v2b_ENTRY_GATE_CONFIRM_LIVE")
    );
    ...
    if (!process.env[`${alias}_AUTO_EXIT_RECONCILIATION_ENABLED`]) childEnv.AUTO_EXIT_RECONCILIATION_ENABLED = envStr("AUTO_EXIT_RECONCILIATION_ENABLED", "true");
